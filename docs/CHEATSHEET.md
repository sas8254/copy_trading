...
```bash
# 1. Login MASTER
python manage.py kite_login --account "mom_zerodha"                       # prints URL
python manage.py kite_login --account "mom_zerodha" --request-token XXXX  # store token
# 2. Login COPY
python manage.py kite_login --account "my_zerodha"
python manage.py kite_login --account "my_zerodha" --request-token YYYY
# 3. Sync lot sizes + restart + check
python manage.py kite_sync_instruments --account "mom_zerodha"
...
docker compose restart celery_worker celery_beat kite_ticker   # reload code into workers
docker compose down                       # stop (keeps data)
docker compose logs -f celery_worker      # watch the copy/reconcile loop
docker compose logs -f kite_ticker        # watch the live order feed
```
...
```bash
python manage.py preflight             # GO / NO-GO readiness
python manage.py kite_positions --account "mom_zerodha"   # live positions
python manage.py send_test_email       # verify SMTP
python manage.py reconcile_selftest    # mismatch→alert→resolve (fake data)
python manage.py dispatch_selftest     # dispatch pipeline (fake data)
```
...
```bash
python manage.py preflight                 # must say GO
